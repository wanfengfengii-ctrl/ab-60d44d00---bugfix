...
- 路径搜索在**整图**上进行（迭代加深：先最短证书数，再按
  叶→根指纹序列字典序），但**不会先固定最短链再查撤销**：
  每条到达锚的完整路径都要同时通过密码学、层级、pathLen、名称约束、
  策略、EKU 和**双时态撤销**裁决。
...
- 幂等表以 `(scope, client_request_id)` 为主键，记录规范化摘要与响应；
  裁决另以规范化请求摘要内容寻址去重。
- 幂等采用**先认领后写入**：在任何领域写入之前，以 `BEGIN IMMEDIATE`
  事务原子认领 `(scope, client_request_id)`。相同标识、规范化内容不同
  的并发请求在任何业务写入发生前即冲突（`409`）；内容相同的请求等待
  处理完成并重放同一响应。认领带租约与心跳：处理实例崩溃后，相同内容
  的重试在租约到期后接管认领——既不会永久阻塞，也因领域操作的确定性
  而不会产生第二份结果。
